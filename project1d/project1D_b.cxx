...
void line(double x1, double x2, double y, Screen screen, double leftZ, double rightZ, double leftRGB[], double rightRGB[])
{
   if (y<0 || y >= screen.height)
	...
	if (z > screen.z_buffer[(int)y*screen.width+i])
	{
	  ...
	  //cerr<< "Assigning pixel (" << i << "," << y << ") with color " << (int)rgb[0] << ", " << (int)rgb[1] << ", " << (int)rgb[2] << " and depth " << z << endl; 
	}
    }
...
void draw(double x1, double y1, double z1, double color1[],  double x2, double y2, double z2, double color2[], double x3, double y3, double z3, double color3[], Screen screen, double calor[][3]){
...
int main()
{
   ...
   float *z_buffer = new float[npixels];
...
#define z1_g  triangle[i].Z[order[0]]
#define z2_g  triangle[i].Z[order[1]]
#define z3_g  triangle[i].Z[order[2]]
#define color1 triangle[i].color[order[0]]
#define color2 triangle[i].color[order[1]]
#define color3 triangle[i].color[order[2]]
#define colors triangle[i].color
   ...
   screen.z_buffer = z_buffer;
      ...
      {
	if (Mx1 < Mx2)
	  draw(Mx3,My3,z3_g,color3, Mx2,My2,z2_g,color2, Mx1,My2,z1_g,color1,  screen,colors);
	else
	  draw(Mx3,My3,z3_g,color3, Mx1,My2,z1_g,color1, Mx2,My2,z2_g,color2,  screen,colors);
      }
      ///flat top
      else if (My2 == My3)
      {
	if (Mx2< Mx3)
	  draw(Mx1,My1,z1_g,color1, Mx2,My2,z2_g,color2, Mx3,My2,z3_g,color3,  screen,colors);
	else
	  draw(Mx1,My1,z1_g,color1, Mx3,My2,z3_g,color3, Mx2,My2,z2_g,color2,  screen,colors);
      }
      ///needs splitting
	...
	{
	  //top-flat
	  draw(Mx1,My1,z1_g,color1,  xNew,My2,zNew,colorNew,  Mx2,My2,z2_g,color2,		screen,colors);
	  draw(Mx3,My3,z3_g,color3,  Mx2,My2,z2_g,color2,  	 xNew,My2,zNew,colorNew,	screen,colors);
	  //bottom-flat
	}
	else
	{
	  //top-flat
	  draw(Mx1,My1,z1_g,color1,  Mx2,My2,z2_g,color2, xNew,My2,zNew,colorNew,		screen,colors);
	  draw(Mx3,My3,z3_g,color3,  Mx2,My2,z2_g,color2, xNew,My2,zNew,colorNew,		screen,colors);
	  //bottom-flat
	}
	times ++ ;
