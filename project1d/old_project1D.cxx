...
void draw(double x1, double y1, double z1, double color1[],  double x2, double y2, double z2, double color2[], double x3, double y3, double z3, double color3[], Screen screen){
  ...
  if (y1<y3){ //top flat triangle
    ...
    for (int i = ceil441(y1); i <= floor441(y3); i++)
    {
      ...
      if (x1 != x3){
	za = z1 + ( (((double)i)-y1) / (y2 - y1) ) * (z2 - z1);
	//for (int j = 0; j < 3; j++)
	  //leftRGB[j] = color1[0] + (((double)i - y1)/(y2-y1)) * (color2[0]-color1[0]);
      }
      else za = z1 + ((right - x1)/(x2-x1)) * (z2- z1);
          
      if (x1 != x2) zb= z1 + (((double)i - y1)/(y3-y1)) * (z3 - z1);
      else zb = z1 + ((right - x1)/(x3-x1)) * (z3-z1);
      
      
      
      for (int j = 0; j < 3; j++)
	leftRGB[j] = (color2[j]-color1[j])/height == 0? leftRGB[j] : color2[j] + (((double)i-y3) * ((color2[j]-color1[j])/height));
  ...
  else //bottom flat triangle
  {
    for (int i = floor441(y1); i>=ceil441(y3); i--)
    {
     ...
     // if (x1 != x2) za =    z1 + ( (((double)i)-y1) / (y2 - y1) ) * (z2 - z1);
      //else za =             z1 + ((left - x1)/(x2-x1)) * (z2- z1);
          
      //if (x1 != x3) zb= 	  z1 + (((double)i - y1)/(y3-y1)) * (z3 - z1);
     // else 	   zb = 	  z1 + ((right - x1)/(x3-x1)) * (z3-z1);
